...
// Play audio on user interaction (e.g., click)
function playAudio() {
	audio.play().catch(error => {
			console.error("Audio play failed:", error);
	});
	// Optionally remove the event listener after the first click to avoid multiple plays
	document.removeEventListener("click", playAudio);
}

// Add click listener to play audio and create fireworks
document.addEventListener("click", (e) => {
	playAudio();
	createFirework(e.clientX, e.clientY);
});

// Automatically trigger a firework and play audio when the page loads
window.onload = function () {
	// Delay playing audio slightly to handle autoplay restrictions
	setTimeout(() => {
			playAudio();
